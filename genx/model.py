...
# GenX libraries
import genx.data
import genx.fom_funcs
import genx.parameters
...
class Model:
    ...
    def __init__(self, config=None):
        ...
        '''
        self.config = config
        self.data = genx.data.DataList()
        self.script = ''
        self.parameters = genx.parameters.Parameters(model=self)

        # self.fom_func = default_fom_func
        self.fom_func = genx.fom_funcs.log  # The function that evaluates the fom
    ...
    def read_h5group(self, group, **kwargs):
        ...
        fom_func_name = group['fomfunction'].value
        if fom_func_name in genx.fom_funcs.func_names:
            self.set_fom_func(eval('genx.fom_funcs.' + fom_func_name))
        else:
            print("Can not find fom function name %s" % fom_func_name)
    ...
    def load_addition(self, name):
        ...
        try:
            text = loadfile.read(name).decode('latin1')
        except Exception as e:
    ...
    def new_model(self):
        ...
        self.data = genx.data.DataList()
        self.script = ''
        self.parameters = genx.parameters.Parameters(self)

        # self.fom_func = default_fom_func
        self.fom_func = genx.fom_funcs.log
        self._reset_module()
    ...
    def get_possible_parameters(self):
        ...
        isstrings = sum([isinstance(p, type('')) for p in pars]) == len(pars)
