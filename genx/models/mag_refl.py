...
from genx.models.lib import refl
import genx.models.lib.xrmr
import genx.models.lib.edm_slicing as edm
try:
...
import genx.models.lib.ables as ables
import genx.models.lib.neutron_refl as neutron_refl
from numpy import *
...
def compose_sld_anal(z, sample, instrument):
    ...
    if (theory == 0 or theory == instrument_string_choices['theory'][0]):
        ...
        chi = genx.models.lib.xrmr.create_chi(None, None, A*0, A, B, C, M, None)[0]
        chi_l = genx.models.lib.xrmr.create_chi(None, None, A*0, A, B*(1 + dmag_l), C*(1 + dmag_l)**2, M, None)[0]
        chi_u = genx.models.lib.xrmr.create_chi(None, None, A*0, A, B*(1 + dmag_u), C*(1 + dmag_u)**2, M, None)[0]
...
def compose_sld(sample, instrument, theta, xray_energy, layer=None):
    ...
    if sample.getSlicing() == sample_string_choices['slicing'][0]:
        ...
        chi, non_mag, mpy = genx.models.lib.xrmr.create_chi(g_0, lamda, A, 0.0*A, B, C, M, d)
        ...
        g_0 = sin(theta*pi/180.0)
        chi, non_mag, mpy = genx.models.lib.xrmr.create_chi(g_0, lamda, A, 0.0*A, 
                                                    B, C, M, d)
    ...
    if theory == 0 or theory == instrument_string_choices['theory'][0]:
        ...
        if theory == 0 or theory == instrument_string_choices['theory'][0]:
            if True or (XBuffer.parameters != parameters or XBuffer.coords != instrument.getCoords()
                ...
                W = genx.models.lib.xrmr.calc_refl_int_lay(g_0, lamda, A*0, A[::-1], B[::-1], C[::-1], M[::-1,...]
            ...
            #Wc = genx.models.lib.xrmr.dot2(trans, lib.xrmr.dot2(W, lib.xrmr.inv2(trans)))
            Wc = genx.models.lib.xrmr.dot2(trans, lib.xrmr.dot2(W, conj(lib.xrmr.inv2(trans))))
...
def slicing_reflectivity(sample, instrument, theta, TwoThetaQz, xray_energy, return_amplitude=True):
    ...
    if theory == 0 or theory == instrument_string_choices['theory'][0]:
        if (XBuffer.parameters != parameters or XBuffer.coords != instrument.getCoords() or
            ...
            non_mag = non_mag[::-1]
            mpy = mpy[::-1]
            W = genx.models.lib.xrmr.do_calc(g_0, lamda, chi, d, non_mag, mpy)
            XBuffer.W = W
            XBuffer.parameters = parameters.copy()
            XBuffer.coords = instrument.getCoords()
            XBuffer.g_0 = g_0.copy()
