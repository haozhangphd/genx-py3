...
import genx.data, genx.model, genx.help
import genx.filehandling as io
import genx.plotpanel, genx.solvergui, genx.parametergrid, genx.datalist
import genx.event_handlers
import genx.images as img

import genx.plugins.add_on_framework as add_on
...
global_config_path = appdirs.site_config_dir('genx', 'MattsBjorck').replace('/xdg', '') + '/'
config_path = appdirs.user_config_dir('genx', 'MattsBjorck') + '/'
print(config_path)
...
if not os.path.exists(config_path + 'profiles'):
    ...
    shutil.copytree(global_config_path + 'profiles', config_path + 'profiles')
if not os.path.exists(config_path + 'genx.conf'):
    print('Creating genx.conf at ', config_path, 'by copying config from ', global_config_path + 'profiles/Default.conf')
    shutil.copyfile(global_config_path + 'profiles/Default.conf', config_path + 'genx.conf')
...
class MainFrame(wx.Frame):
    def __init__(self, parent, show_startup, *args, **kwds):
        ...
        config_loaded = False
        for loc in config_path, os.path.expanduser("~"), global_config_path:
            try: 
                self.config.load_default( os.path.join(loc,'genx.conf') )
            except IOError:
                pass
            else:
                config_loaded = True
        ...
        status_text = lambda event:genx.event_handlers.status_text(self, event)
        ...
        self.mb_edit.Append(self.mb_copy_graph)
        ...
        self.mb_edit_sub.Append(self.mb_new_data_set)
        ...
        self.mb_edit_sub.Append(self.mb_toggle_show)
        ...
        self.mb_edit_sub.Append(self.mb_toggle_use)
        ...
        self.mb_edit_sub.Append(self.mb_toggle_error)
        self.mb_edit_sub.AppendSeparator()
        ...
        self.mb_edit_sub.Append(self.mb_toggle_calc)
        self.mb_edit.Append(wx.NewId(), "Data", self.mb_edit_sub, "")
        ...
        mb_view_yscale.Append(self.mb_view_yscale_log)
        self.mb_view_yscale_lin = wx.MenuItem(mb_view_yscale, wx.NewId(), "lin", "Set y-scale linear", wx.ITEM_RADIO)
        mb_view_yscale.Append(self.mb_view_yscale_lin)
        self.mb_view.Append(wx.NewId(), "y scale", mb_view_yscale, "")
        ...
        self.mb_view.Append(self.mb_use_toggle_show)
        ...
        self.mb_view_grid_slider = wx.MenuItem(mb_view_grid, wx.NewId(), "Value as slider",
                                          ...
                                          wx.ITEM_CHECK)
        mb_view_grid.Append(self.mb_view_grid_slider)
        self.mb_view.Append(wx.NewId(), "Grid", mb_view_grid, "")
        self.mb_fit = wx.Menu()
        ...
        self.mb_fit.Append(self.mb_fit_simulate)
        ...
        self.main_frame_toolbar.AddTool(10009, "tb_zoom", (img.getzoomBitmap()), wx.NullBitmap, wx.ITEM_CHECK, "Zoom | Ctrl+Z", "Turn the zoom on/off  | Ctrl+Z")
        # Tool Bar end
        self.data_list = genx.datalist.DataListControl(self.data_notebook_data, -1, self.config, status_text)
        ...
        self.plot_data = genx.plotpanel.DataPlotPanel(self.plot_notebook, config = self.config, config_name = 'data plot', )
        self.plot_fom = genx.plotpanel.ErrorPlotPanel(self.plot_notebook, config = self.config, config_name = 'fom plot', )
        self.plot_pars = genx.plotpanel.ParsPlotPanel(self.plot_notebook, config = self.config, config_name = 'pars plot', )
        self.plot_fomscan = genx.plotpanel.FomScanPlotPanel(self.plot_notebook, config = self.config, config_name = 'fom scan plot', )
        self.paramter_grid = genx.parametergrid.ParameterGrid(self.input_notebook, self, self.config)
        ...
        self.example_handler = genx.help.ExampleHandler(self, self.mb_examples, library_path + 'examples/')
        ...
        self.model = genx.model.Model(config=self.config)
        self.solver_control = genx.solvergui.SolverController(self, self.config)
        ...
        self.Bind(genx.event_handlers.EVT_NEW_MODEL, self.paramter_grid.OnNewModel, self)
        self.Bind(genx.event_handlers.EVT_NEW_MODEL, self.data_list.eh_external_new_model, self)
        # Update the script
        self.Bind(genx.event_handlers.EVT_NEW_MODEL, self.eh_new_model, self)
        # Event that the plot should respond to
        self.Bind(genx.datalist.EVT_DATA_LIST, self.plot_data.OnDataListEvent, self.data_list.list_ctrl)
        self.Bind(genx.datalist.EVT_DATA_LIST, self.eh_external_update_data_grid_choice, self.data_list.list_ctrl)
        self.Bind(genx.datalist.EVT_DATA_LIST, self.eh_external_update_data, self.data_list.list_ctrl)
        
        self.Bind(genx.event_handlers.EVT_SIM_PLOT, self.plot_data.OnSimPlotEvent,\
                    self)
        self.Bind(genx.event_handlers.EVT_SIM_PLOT, self.eh_external_fom_value,\
                    self)
        # Update events from the solver
        self.Bind(genx.solvergui.EVT_UPDATE_PLOT, self.eh_external_fom_value)
        self.Bind(genx.solvergui.EVT_UPDATE_PLOT, self.plot_data.OnSolverPlotEvent)
        self.Bind(genx.solvergui.EVT_UPDATE_PLOT, self.plot_fom.OnSolverPlotEvent)
        
        self.Bind(genx.solvergui.EVT_SOLVER_UPDATE_TEXT, \
                            self.eh_ex_status_text)
        self.Bind(genx.solvergui.EVT_UPDATE_PARAMETERS,\
                            self.paramter_grid.OnSolverUpdateEvent)
        self.Bind(genx.solvergui.EVT_UPDATE_PARAMETERS,\
                            self.plot_pars.OnSolverParameterEvent)
        # For picking a point in a plot
        self.Bind(genx.plotpanel.EVT_PLOT_POSITION,\
        ...
        proj_func = lambda row: genx.event_handlers.project_fom_parameter(self, row)
        scan_func = lambda row: genx.event_handlers.scan_parameter(self, row)
        ...
        try:
            ...
            genx.event_handlers.ShowErrorDialog(self, 'Could not read the config for the plots. Python Error:\n%s'%(val,))

        genx.event_handlers.new(self, None)
    ...
    def startup_dialog(self, config_path, force_show = False):
        show_profiles = self.config.get_boolean('startup', 'show profiles')
        if show_profiles or force_show:
            startup_dialog = StartUpConfigDialog(self, config_path + 'profiles/', show_cb = show_profiles)
            startup_dialog.ShowModal()
            config_file = startup_dialog.GetConfigFile()
            if config_file:
                shutil.copyfile(config_path + 'profiles/' + config_file, config_path + 'genx.conf')
    ...
    def eh_mb_misc_showman(self, event): # wxGlade: MainFrame.<event_handler>
        genx.event_handlers.show_manual(self, event)

    def eh_mb_misc_about(self, event): # wxGlade: MainFrame.<event_handler>
        genx.event_handlers.show_about_box(self, event)

    def eh_data_grid_choice(self, event): # wxGlade: MainFrame.<event_handler>
        genx.event_handlers.change_data_grid_view(self, event)
    ...
    def eh_tb_simulate(self, event): # wxGlade: MainFrame.<event_handler>
        #event.Skip()
        genx.event_handlers.simulate(self, event)
    ...
    def eh_mb_misc_openhomepage(self, event): # wxGlade: MainFrame.<event_handler>
        ...
        #event.Skip()
        genx.event_handlers.show_homepage(self, event)
    ...
    def eh_external_parameter_value_changed(self, event):
        genx.event_handlers.parameter_value_changed(self, event)
