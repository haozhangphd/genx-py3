...
import genx.model as mmodel

from genx.lib.Simplex import Simplex
...
class DiffEv:
    ...
    def simplex_old_init_new_generation(self, gen):
        ...
        if gen%self.simplex_interval == 0:
            ...
            for index1 in random_mod.sample(list(range(len(self.pop_vec))),
...
class CircBuffer:
    ...
    def copy_from(self, object):
        '''Add copy support
        '''
        if isinstance(object, type(array([]))):
            self.buffer = object[-self.maxlen:]
            self.pos =  len(self.buffer) - 1
            self.filled = self.pos >= (self.maxlen - 1)
        elif object.__class__ == self.__class__ or str(object.__class__) == str(self.__class__).replace('genx.', ''):
