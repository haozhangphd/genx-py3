...
import genx.plugins.add_on_framework as framework
import genx.plugins.add_ons.help_modules.reflectivity_images as images
from genx.plotpanel import PlotPanel
from genx.plugins.add_ons.help_modules.custom_dialog import *
            ...
            if self.poslist[pos][1] == None:  # Moving a stack
                if self.poslist[pos][0] != 0:
                    ...
                    for index in range(len(temp.Layers) + 1):
                        temps.append(self.names.pop(pos))
                    for index in range(len(temp.Layers) + 1):
                        self.names.insert(pos + len(self.sample.Stacks[self.poslist[pos][0] - 1].Layers) + 1,
                                          temps[-index - 1])
                    self.sample.Stacks.insert(self.poslist[pos][0] - 1, temp)
                    return self.getStringList()
                else:
...
class Plugin(framework.Template):
    ...
    def CreateNewModel(self, modelname='models.spec_nx'):
        ...
        script += 'from genx.models.utils import UserVars, fp, fw, bc, bw\n\n'
    ...
    def WriteModel(self):
        ...
        code = 'from genx.models.utils import create_fp, create_fw\n'
...
if __name__ == '__main__':
    import genx.models.interdiff as Model

    nSi = 3.0
    Fe = Model.Layer(d=10, sigmar=3.0, f=1 - 2.247e-5 + 2.891e-6j)
    Si = Model.Layer(d=15, sigmar=3.0, f='nSi')
    sub = Model.Layer(sigmar=3.0, f=1 - 7.577e-6 + 1.756e-7j)
    amb = Model.Layer(f=1.0)
    ...
    print(sample)
    inst = Model.Instrument(wavelength=1.54, coords=1)
