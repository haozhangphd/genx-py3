        ...
        for par in self.export_parameters:
            ...
            if isinstance(obj, dict):
                ...
                for key in sub_group:
                    ...
                    elif isinstance(sub_group[par].value, bytes):
                        obj[key] = self.export_parameters[par](sub_group[key].value.decode('latin1'))
                    else:
                        obj[key] = self.export_parameters[par](sub_group[key].value)
            else:
                if self.export_parameters[par] is array:
                    setattr(self, par, group[par].value)
                elif isinstance(group[par].value, bytes):
                    setattr(self, par, self.export_parameters[par](group[par].value.decode('latin1')))
                else:
